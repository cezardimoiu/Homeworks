...
public class SpaceExplorer extends Thread {
	...
	public void run() {
		while (true) {
			Message msg = channel.getMessageHeadQuarterChannel();
			if (msg != null && msg.getData().equals("WRong") == false) {
				if (!discovered.contains(msg.getCurrentSolarSystem())) {
					if (msg.getData() != null) {
						String decoded = encryptMultipleTimes(msg.getData(), hashCount);
						msg.setData(decoded);
					}
					synchronized (discovered) {
						discovered.add(msg.getCurrentSolarSystem());
					}
					channel.putMessageSpaceExplorerChannel(msg);
				}
			}
		}
	}
